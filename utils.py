...
def get_random_directions(num_samples):
    ...
    # return 2.5 * random_directions

def get_random_lights(num_samples):
    random_lights = np.random.randn(num_samples, 3)
    random_lights /= np.linalg.norm(random_lights, axis=1).reshape(-1, 1)
...
def load_pretrained_nerf_model(dev, cfg):
    ...
    pretrained_nerf, embed_fn, embeddirs_fn, embedlights_fn = create_nerf(pretrained_cfg)
    ...
    pretrained_nerf = run_nerf_helpers.ChainEmbeddingAndModel(pretrained_nerf.model_coarse, embed_fn, embeddirs_fn, embedlights_fn) # pos. encoding
...
def create_nerf(cfg):
    ...
    embedlights_fn, input_ch_lights = run_nerf_helpers.get_embedder(cfg['num_frequencies_direction'], 0)
    output_ch = 4
    skips = [cfg['refeed_position_index']]
    model = run_nerf_helpers.NeRF2(D=cfg['num_hidden_layers'], W=cfg['hidden_layer_size'],
    ...
    if cfg['num_importance_samples_per_ray'] > 0:
        model_fine = run_nerf_helpers.NeRF2(D=cfg['num_hidden_layers'], W=cfg['hidden_layer_size'],
                          input_ch=input_ch, output_ch=output_ch, skips=skips,
                          input_ch_views=input_ch_views, input_ch_lights=input_ch_lights,
                          use_viewdirs=True, use_lightdirs=True,
                          direction_layer_size=cfg['direction_layer_size'], use_initialization_fix=cfg['use_initialization_fix'])
    ...
    return model, embed_fn, embeddirs_fn, embedlights_fn
